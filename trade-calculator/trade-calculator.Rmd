...
Wir sehen, dass die Values so nicht wirklich nutzbar sind - denn wenn Kicker nicht den geringsten Value haben, haben wir ein Problem. Da kommt Positional Value ins Spiel.
...
positional_value <- starter_data_raw %>% 
  ...
  dplyr::group_by(pos) %>% 
  dplyr::summarise(
    dplyr::across(position_count:position_value, mean, na.rm = TRUE),
    .groups = "drop"
  )
