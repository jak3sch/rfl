...
recent_mvps <- league_awards %>% 
  ...
  dplyr::mutate(
    war_pct = (war / max(war)) * 7,
    size_outer_ring = 35 + war_pct,
...
chart <- ggplot(recent_mvps, aes(x = 0.5, y = 0.8)) +
  ...
  geom_image(aes(image = cropcircles::circle_crop(headshot, border_size = 20, border_colour = var.colorBlue)), asp = 1.25, size = 0.35) +
...
```{r mvp history, echo=FALSE, message=FALSE, warning=FALSE}
past_mvp_data <- league_awards %>% 
  dplyr::filter(award == "MVP" & rank == 1) %>% 
  dplyr::mutate(
    segment_start = ifelse(war >= mean(war), war - 0.13, war + 0.13),
...
chart <- ggplot(past_mvp_data, aes(x = war, y = season)) +
  ...
  geom_image(data = subset(past_mvp_data, season != var.lastSeason), aes(image = cropcircles::circle_crop(headshot, border_size = 20, border_colour = var.colorAccent)), asp = 2.4, size = 0.08) +
  geom_image(data = subset(past_mvp_data, season == var.lastSeason), aes(image = cropcircles::circle_crop(headshot, border_size = 20, border_colour = var.colorYellow)), asp = 2.4, size = 0.08) +
...
### Position MVPs
```{r league-position-mvps, echo=FALSE, fig.height=7.5, fig.width=13, message=FALSE, warning=FALSE}
...
chart <- ggplot(subset(league_mvp_positions, rank <= 3), aes(x = factor(rank, levels = c(2,1,3)), y = colHeight)) +
  ...
  geom_text(aes(label = war), vjust = 1.7, size = plot.geom_text_large_size, color = plot.geom_text_large_color, family = plot.geom_text_large_family) +
...
## Starter
### Starter by Position
```{r starter by position, echo=FALSE}
...
```{r starter per nfl team, echo=FALSE, message=FALSE, warning=FALSE}
...
chart_data <- starter_by_nfl_team_data %>% 
  ...
  dplyr::mutate(per_starter = ifelse(is.na(starter), total, total / starter))
...
chart2 <- starter_per_nfl_team_base_chart +
  ...
  geom_text(data = latest_chart_data, aes(label = round(per_starter)), size = plot.geom_text_xsmall_size, family = plot.geom_text_xsmall_family, color = var.colorAccent) +
  labs(
    title = toupper(paste("Die Fantasy Studs der NFL Teams", var.lastSeason)),
    subtitle = paste(var.lastSeason, "gestartete Spieler / im durchschnitt gestartete Spieler")
  )
...
# Conferences
```{r conferences, echo=FALSE, message=FALSE, warning=TRUE, include=FALSE}
...
```{r div win perc by conf, eval=FALSE, message=FALSE, warning=TRUE, include=FALSE}
...
chart1 <- ggplot(subset(heatmapChartData, season == var.lastSeason), aes(y = rank_division, x = div_rank_in_conf, fill = win_pct)) +
  ...
  chartDefaults +
  labs(
    title = paste("Division Win Pct", var.lastSeason),
    subtitle = paste("Verteilung der Win Percentages\nin den Division nach Conference in", var.lastSeason),
    fill = "Win %"
  ) 
...
chart2 <- ggplot(subset(heatmapChartData, season == var.lastSeason), aes(y = rank_division, x = div_rank_in_conf, fill = allplay_winpct)) +
  ...
  chartDefaults +
  labs(
    title = paste("Division All Play Win Pct", var.lastSeason),
    subtitle = paste("Verteilung der All Play Win Percentages\nin den Division nach Conference in", var.lastSeason),
    fill = "All Play Win %"
  )
...
chart <- ggplot(standings, aes(x = win_pct)) +
  geom_density(color = "transparent", fill = var.colorAccent, alpha = .2) + 
  geom_density(data = confWinsLastYear, aes(color = paste(conference_name, var.lastSeason)), alpha = 0, size = 1, linetype = "dashed") +
  geom_density(data = subset(standings, season == var.lastSeason), aes(color = paste(conference_name, var.lastSeason)), alpha = 0, linewidth = 1) +
  
  scale_color_manual(values = c(var.colorRed, var.colorYellow)) +
  plotHistogram +
  labs(
    title = "Conference Win Pct - Histogram",
...
for (row in 1:nrow(latestConferences)) {
  ...
  chart <- ggplot(subset(heatmapChartDataGathered, season == var.lastSeason & conference_id == confID$id), aes(y = rank_division, x = reorder(key, win_pct), fill = win_pct)) +
    chartDefaults +
    labs(
      title = paste(confName, "Division Win Pct", var.lastSeason),
...
# Divisions
```{r divisions, echo=FALSE, message=FALSE, warning=TRUE, include=FALSE}
output <- output %>% 
  officer::add_slide(layout = "Kapitel") %>% 
...
## Division Siege
```{r divs win pct, eval=FALSE, message=FALSE, warning=TRUE, include=FALSE}
# heat map
chart <- ggplot(standings, aes(x = rank_division, y = season, fill = win_pct)) +
  facet_wrap(vars(division_name)) +
  plotHeatmap +
  labs(
    title = "Division Win Pct - Heatmap",
...
chartWins <- ggplot(standings, aes(x = win_pct, group = 1)) +
  ...
  geom_density(color = "transparent", fill = var.colorAccent, alpha = .2) +
  geom_density(data = subset(standings, season == var.lastSeason), aes(fill = paste("Saison", var.lastSeason)),
               color = var.colorRed, alpha = 0) +
  plotHistogram +
  labs(
    title = "Division Win Pct - Histogram",
    subtitle = paste("Verteilung der Win Percentages innerhalb der Divisions ", var.firstSeason, "-", var.lastSeason, " (Reg Season)", sep = ""),
    x = "Win Percentage"
  ) +
  theme(
    legend.position = "top"
...
chartPoints <- ggplot(standings, aes(x = points_for / 2)) +
  ...
  plotHistogram +
  labs(
    title = "Division Total PF - Histogram",
    subtitle = paste("Verteilung der Total Points innerhalb der Divisions", var.firstSeason, "-", var.lastSeason, "(Reg Season)"),
    x = "Total Points"
  ) +
  theme(
    legend.position = "top"
...
```{r divs kumuliert, eval=FALSE, message=FALSE, warning=TRUE, include=FALSE}
divTotals <- divisionStandings %>% 
  ...
  mutate(
    total_div_points = format(round(as.numeric(sum(points_for)), 0), nsmall = 0, big.mark = ","),
    label_wins = paste(division_name, "\nTotal:\n", sum(h2h_wins), sep = ""),
    label_points = paste(division_name, "\nTotal:\n", total_div_points, sep = "")
  ) %>% 
  ungroup()
...
kumulierteChartsDefaults <- list(
  geom_col(),
  geom_text(position = position_stack(vjust = 0.5), size = 4, color = var.colorBlue),
  plotDefaults,
  paletteer::scale_fill_paletteer_d("colorBlindness::ModifiedSpectralScheme11Steps"),
  theme(
    legend.position = "top",
    plot.margin = margin(15, 15, 5, 15),
  ),
...
chart1 <- ggplot(divTotals,
                aes(y = h2h_wins, x = reorder(label_wins, h2h_wins),
                    fill = season, label = percent(division_win_pct, accuracy = 0.1))) +
  
  kumulierteChartsDefaults +
  labs(
    title = "Kumulierte Siege",
    subtitle = paste("Alle Wins der Divisions von", var.firstSeason, "-", var.lastSeason, "addiert (Reg Season)"),
    y = "Anzahl der Siege",
    x = "",
    fill = ""
  ) +
...
# kumulierte Punkte
chart2 <- ggplot(divTotals, aes(y = points_for, x = reorder(label_points, h2h_wins), fill = season, label = percent(division_points_for_pct, accuracy = 0.1))) +
  kumulierteChartsDefaults +
  labs(
    title = "Kumulierte Total Points",
    subtitle = paste("Alle Total Points von ", var.firstSeason, "-", var.lastSeason, " addiert (Reg Season)", sep = ""),
    y = "Total Points",
    x = "",
    fill = ""
  ) +
  scale_y_continuous(breaks = pretty_breaks(), labels = scales::comma, expand=c(0,0,0,100))
...
## einzelne Division
```{r div, eval=FALSE, message=FALSE, warning=TRUE, include=FALSE}
divs <- divInConf %>% 
...
for (row in 1:nrow(divs)) {
  divID <- divs[row, "division"]
  divID <- divID$division
  divName <- divs[row, "division_name"]
  divName <- divName$division_name
  ...
  chart <- ggplot(data = subset(heatmapChartDataGathered, season == var.lastSeason & division == divID),
    ...
    geom_text(aes(label = paste("W", h2h_wins, "-", h2h_losses, "L\n", format(round(as.numeric(points_for), 2), big.mark = ","), "PF\nExpW: ", round(sf_expw, 0)), color = h2h_wins > 11), x = "Win Pct") +
    geom_text(aes(label = paste("W ", allplay_wins, " - ", allplay_losses, " L\nLuck Pctl Season\n", percent(sf_luck_pctl_season, 2), " (", luck_term, ")", sep =""), color = allplay_wins > 290), x = "All Play Win Pct") +
  ...
  chart2 <- ggplot(chartData, aes(x = matchup, y = sos, group = franchise_name)) +
    plotDefaults +
    geom_point(data = subset(chartData, division != divID), color = var.colorAccent, size = 2, alpha = 0.3) +
    geom_bump(data = subset(chartData, division == divID), aes(color = franchise_name), size = 1) +
    geom_point(data = subset(chartData, division == divID), aes(color = franchise_name), size = 3) +
    paletteer::scale_color_paletteer_d("awtools::ppalette") +
    theme(
      plot.margin = margin(15, 15, 5, 15),
      legend.position = "top",
    ...
    labs(
      title = paste(divName, "SOS", var.lastSeason),
