...
```{r variables, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
var.platform = "mfl"
...
# Einleitung
```{r einleitung, message=FALSE, warning=TRUE}
output <- output %>% 
...
```{r league-player-awards, fig.height=7.5, fig.width=13, message=FALSE, warning=FALSE}
# alle awards ----
...
```{r mvp history, message=FALSE, warning=FALSE}
...
### Position MVPs
```{r league-position-mvps, fig.height=7.5, fig.width=13, message=FALSE, warning=FALSE}
...
```{r starter per nfl team, message=FALSE, warning=FALSE}
...
# Conferences
```{r conferences, message=FALSE, warning=TRUE}
output <- output %>% 
  officer::add_slide(layout = "Kapitel") %>% 
...
```{r confs win perc, message=FALSE, warning=TRUE}
...
```{r div win perc by conf, message=FALSE, warning=TRUE}
...
# Divisions
```{r divisions, message=FALSE, warning=TRUE}
output <- output %>% 
  officer::add_slide(layout = "Kapitel") %>% 
...
## Division Siege
```{r divs win pct, message=FALSE, warning=TRUE}
# heat map
...
```{r divs kumuliert, message=FALSE, warning=TRUE}
...
## einzelne Division
```{r div, message=FALSE, warning=TRUE}
...
# Teams
```{r teams kapitel, echo=FALSE, message=FALSE, warning=TRUE}
output <- output %>% 
  officer::add_slide(layout = "Kapitel") %>% 
...
```{r team, echo=FALSE, message=FALSE, warning=TRUE}
# points by position
...
for (i in franchisesLatestIDs) {
  ...
  key_order <- c("Win %", "All Win %", "Expected Wins", "Total Points", "Potential Points", "Points per Game", "Points Against", "Effizienz", "Luck", "Skill")
  ...
  chart1 <- ggplot(points_per_game_data, aes(x = week, y = wins, alpha = weekly_pts_pct)) +
    ...
    labs(
      title = toupper(paste0(franchise$franchise_name, ": Siege pro Spieltag")),
      alpha = ""
    ) +
    theme(
      strip.text = element_text(color = plot.theme_text_shadow_color, size = plot.theme_text_shadow_size, family = plot.theme_text_shadow_family),
  ...
  chart_annotations <- chart_data %>% 
    ...
    dplyr::transmute(annotations = pmap(list(season, x_mean, y_min), function(s, x, y) {
      annotate("text", x = x, y = y - 50, label = s, color = plot.geom_text_shadow_color, size = plot.geom_text_shadow_size, family = plot.geom_text_shadow_family)
    })) %>% 
    pull(annotations)
  ...
  chart2 <- ggplot(chart_data, aes(x = game_nr, y = franchise_elo_postgame, group = franchise_id)) +
    ...
    geom_hline(yintercept = 1500, color = var.colorLightblue, size = plot.line_width) +
    geom_line(color = var.colorAccent, alpha = 0.1, size = plot.line_width_m) +
    geom_line(data = subset(chart_data, franchise_id == i), color = var.colorYellow, linewidth = plot.line_width_l) +
    ...
    geom_text(data = subset(biggest_elo_changes_chart_data, biggest_win == 1), aes(y = yLabel, label = "Größte Überraschung vs."), color = var.colorAccent, size = plot.geom_text_xsmall_size) +
    geom_text(data = subset(biggest_elo_changes_chart_data, biggest_win == 1), aes(y = yLabelLarge, label = paste0(franchise_name, " +", elo_shift)), color = plot.geom_text_large_color, size = plot.geom_text_large_size, family = plot.geom_text_large_family) +
    ...
    geom_text(data = subset(biggest_elo_changes_chart_data, biggest_loss == 1), aes(y = yLabel, label = "Größte Enttäuschung vs."), color = var.colorAccent, size = plot.geom_text_xsmall_size) +
    geom_text(data = subset(biggest_elo_changes_chart_data, biggest_loss == 1), aes(y = yLabelLarge, label = paste(franchise_name, elo_shift)), color = plot.geom_text_large_color, size = plot.geom_text_large_size, family = plot.geom_text_large_family) +
  
    
    labs(
      title = toupper(paste0(franchise$franchise_name, ": ELO Entwicklung ", var.firstSeason, " - ", var.lastSeason))
    ) +
    theme(
      panel.spacing = unit(0, "lines"),
      axis.text.y = element_text(vjust = 0.4, size = plot.theme_text_shadow_size, family = plot.theme_text_shadow_family, color = plot.theme_text_shadow_color),
    )
...
#  
  ...
  i <- "0007"
  write.csv(rfl_personnel %>% filter(franchise_id == i), "output/plots/team-personnel-grouping/data.csv", row.names = F)
  write.csv(franchise_personnel_groupings_annotations, "output/plots/team-personnel-grouping/annotations.csv", row.names = F)
  
  franchise_personnel_groupings_annotations <- rfl_personnel_annotations %>% 
    dplyr::filter(franchise_id == i) %>% 
    select(-middle, -order)
  
  offense_chart <- plot_personnell_groupings(
    data = subset(rfl_personnel, franchise_id == i & unit == "offense"),
    title = paste0(franchise$franchise_name, ": Personnel Groupings - Offense"),
    subtitle = "Verteilung der gestarteten Offense Spieler"
  )
  
  defense_chart <- plot_personnell_groupings(
    data = subset(rfl_personnel, franchise_id == i & unit == "defense"),
    title = paste0(franchise$franchise_name, ": Personnel Groupings - Defense"),
    subtitle = "Verteilung der gestarteten Defense Spieler"
  )
  
  for(pos in as.vector(unique(franchise_personnel_groupings_annotations$position))) {
    offense_chart <- offense_chart +
      plot_personnell_groupings_points(
        data = subset(franchise_personnel_groupings_annotations, unit == "offense"),
        pos = pos
    )
    
    defense_chart <- defense_chart +
      plot_personnell_groupings_points(
        data = subset(franchise_personnel_groupings_annotations, unit == "defense"),
        pos = pos
    )
  }
  ...
  check <- rfl_personnel %>% 
    dplyr::select(-season, -count) %>% 
    distinct() %>% 
    group_by(franchise_id, unit)
    summarise(count = n())
  
  output <- output %>% 
    officer::add_slide(layout = "2 Bilder") %>% 
    officer::ph_with(value = offense_chart, location = officer::ph_location_label("img-left")) %>% 
    officer::ph_with(value = defense_chart, location = officer::ph_location_label("img-right"))
  
  rm(franchise_personnel_groupings_annotations, offense_chart, defense_chart)
}
```
