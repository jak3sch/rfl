...
#  i = "0007"
  franchise_personnel_groupings_total <- rfl_personnel %>% 
    dplyr::filter(franchise_id == i)
  
  franchise_personnel_groupings_annotations <- rfl_personnel_annotations2 %>% 
    dplyr::filter(franchise_id == i)
  
  var.spacing <- 0.06
  
  franchise_personnel_groupings_points <- franchise_personnel_groupings_annotations %>% 
    tidyr::gather(position, count, c("RB", "WR", "TE")) %>% 
    dplyr::slice(rep(row_number(), count)) %>% 
    dplyr::group_by(season, unit, personnel, grouping, position) %>%
    dplyr::mutate(
      prio = dplyr::row_number(),
      middle = count / 2,
      x = (xcenter + (var.spacing * prio)),
      xoffset = x - ((var.spacing * count) / 2) - 0.034, # wert durch rumprobieren
      is_to_big = ifelse(
        xoffset > xmax | xoffset < xmin, 1, 0
      ),
      y = case_when(
        position == "RB" ~ ycenter - var.spacing,
        position == "WR" ~ ycenter + var.spacing,
        TRUE ~ ycenter
      )
    ) %>% 
    dplyr::group_by(season, unit, personnel, grouping) %>% 
    dplyr::mutate(is_to_big = sum(is_to_big, na.rm = TRUE)) %>% 
    dplyr::filter(is_to_big <= 0) %>% 
    dplyr::ungroup()
  

  plot_personnell_groupings(
    data = subset(franchise_personnel_groupings_total, unit == "offense")
  ) +
    ...
    geom_point(
      data = subset(franchise_personnel_groupings_points, unit == "offense" & position == "WR"),
      aes(x = xoffset, y = y, color = grouping),
      size = 1.3,
      shape = 16
    ) +
    
    geom_point(
      data = subset(franchise_personnel_groupings_points, unit == "offense" & position == "TE"),
      aes(x = xoffset, y = y, color = grouping),
      size = 1.3,
      shape = 15
    ) +
    ...
    geom_point(
      data = subset(franchise_personnel_groupings_points, unit == "offense" & position == "RB"),
      aes(x = xoffset, y = y, color = grouping),
      size = 1.3,
      shape = 17
    )
  
    #geom_treemap_text(data = subset(franchise_personnel_groupings_annotations, unit = "offense"), aes(label = personnel), color = "white", place = "top")
    
    geom_text(data = subset(franchise_personnel_groupings_labels, unit == "offense"), aes(label = annotate_season, x = 0.2, y = annotate_ystart), vjust = 1)
  
    geom_point(
      data = subset(franchise_personnel_groupings_annotations, unit == "offense"),
      aes(x = annotate_xstart, y = 0.2, fill = grouping),
      color = var.colorBlue,
      size = plot.line_width_l
    )
...
```{r include=FALSE}
    ...
    dplyr::group_by(season, unit) %>% 
    dplyr::mutate(
      # default dimensions of each area
      default_width = xmax - xmin,
      default_height = ymax- ymin,
      
      # multiplikator um default width auf 1 zu normalisieren
      width_fraction = 1 / sum(default_width),
      height_fraction = 1 / sum(default_height),
      
      new_width = default_width * width_fraction,
      new_height = default_height * height_fraction
    )
