...
starters <- ffscrapr::ff_starters(rfl, season = 2021, weeks = 1:13) %>% 
  distinct()
scores <- ffscrapr::ff_playerscores(rfl, season = 2021, week = 1:13)
...
war <- scores %>% 
  ...
  mutate(
    ...
    war_per_points = war / points
  ) %>% 
  left_join(scores %>% select(player_id, player_name, pos), by = "player_id") %>% 
  distinct() %>% 
  select(player_id, player_name, pos, points, war, war_per_points)

write.csv(war, "data/rfl-war.csv", row.names = F)
