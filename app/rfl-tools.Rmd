...
var.season <- 2022
var.mflApiBase <- "https://www45.myfantasyleague.com/2023/"
...
rfl_draft_history <- rbind(rfl_drafts, draft_data) %>% 
  ...
  dplyr::ungroup() %>% 
  dplyr::filter(overall <= 36 & position == "TE")

ggplot(rfl_draft_history, aes(y = pos_rank, color = as.character(season))) +
  facet_wrap(~season) +
  geom_segment(aes(x = first_pick, xend = third_pick, yend = pos_rank)) +
  geom_point(aes(x = first_pick)) +
  geom_point(aes(x = second_pick)) +
  geom_point(aes(x = third_pick)) +
  scale_y_reverse() +
  labs(
    title = paste("First Round", rfl_draft_history$position[1]),
    x = "OVerall Pick",
    y = "Position Rank"
  ) +
  theme(
