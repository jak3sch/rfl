...
sliderInput("warTeamsMinStarts", "Minimum Starts eines Spielers in einem Team", min = 0, max = 13, value = 7, sep = "")
```
...
## Row
### Elo Peak seit 2016 (Top 25) {data-width=500}
...
DT::renderDataTable({
  ...
  formattable::as.datatable(escape = FALSE, rownames = FALSE, options = list(pageLength = 12, order = list(4, 'asc')))
})
```

### Wochen als Stud (über Trheshold) {data-width=300}
...
trades_data <- trades %>%
  ...
  dplyr::mutate(
    ...
    asset = ifelse(!is.na(draft_year), paste("DP", draft_round, sep = "_"), asset)
  )

selection <- trades_data %>% 
...
selectizeInput(
  "tradeHistoryAssets",
  "Filter nach Asset",
  choices = setNames(selection$asset, selection$name),
  multiple= TRUE,
  options = list(
...
# TODO: zeitraum durch input begrenzen

```

## Row
### Trades mit gewähltem Asset {data-width=800}

```{r trade-history, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE}
DT::renderDataTable({
  table_data <- trades_data %>% 
    dplyr::mutate(
      name = ifelse(grepl("DP_", asset), paste(draft_year, name), name)
    ) %>% 
    dplyr::group_by(trade_id) %>%
    dplyr::mutate(
      asset_ids = paste(asset, collapse = ",")
    ) %>% 
    dplyr::group_by(trade_id, franchise) %>%
    dplyr::summarise(
      date = dplyr::first(date),
      asset_ids = dplyr::first(asset_ids),
      asset_names = paste(name, collapse = "\n"),
      .groups = "drop"
    ) %>% 
    #dplyr::filter(grepl("12655", as.character(asset_ids))) %>% 
    dplyr::filter(grepl(input$tradeHistoryAssets, as.character(asset_ids))) %>% 
    dplyr::select(trade_id, date, franchise, asset_names) %>% 
    tidyr::spread(franchise, asset_names) %>% 
  ...
  DT::datatable(
    table_data,
    rownames = FALSE, options = list(dom = "Bfrtip", pageLength = 12)
  )
})
```
