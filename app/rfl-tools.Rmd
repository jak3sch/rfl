...
# ELO Rating {data-navmenu="Fantasy Insights"}
## Inputs {.sidebar}
...
renderPlot({
  running_elo %>% 
    ggplot(aes(x = game, y = franchise_elo_postgame, color = franchise_name)) +
      ...
      scale_x_discrete(limits = c(1, max(running_elo$games)))
})
```

### ELO Ranking {data-width=300}
...
DT::renderDataTable({
  DT::datatable(
    running_elo %>% 
      ...
      select(franchise_name, franchise_elo_postgame) %>% 
      rename(Franchise = franchise_name, ELO= franchise_elo_postgame),
    rownames = F, options = list(pageLength = 18, order = list(1, 'desc'))
  )
})
```

## Row
### Aktuelle Matchups {data-width=450}
> Win % aus Sicht des Home Teams, berechent anhand der derzeitigen ELO

```{r elo-matchups, echo=FALSE, fig.height=12, message=FALSE, warning=FALSE}
DT::renderDataTable(
  DT::datatable(
    elo_matchups,
    rownames = F, options = list(pageLength = 18, order = list(2, 'desc'))
  )
)
```
...
DT::renderDataTable(
  DT::datatable(
    elo_upsets,
    rownames = F, options = list(pageLength = 18, order = list(5, 'asc'))
  )
)
```
...
sliderInput("selectWeek", "Weeks:", min = 1, max = 13, value = c(1, 3))
