...
DT::renderDataTable({
  warDT <- war %>% 
    select(player_name, pos, points, war, war_per_points) %>% 
    filter(
      pos %in% input$selectPosition
    ) %>% 
    mutate(
      war = round(war, 2),
      war_per_points = round(war_per_points, 5)
    ) %>% 
    rename(
      Spieler = player_name,
      Position = pos,
      FPts = points,
      WAR = war,
      "WAR/FPts" = war_per_points
    )
  
  DT::datatable(
    warDT,
    rownames = F, options = list(pageLength = 50, order = list(3, 'desc'))
  )
})
